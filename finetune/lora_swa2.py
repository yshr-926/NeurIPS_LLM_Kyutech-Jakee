...
def main(
    ...
    if lr_type == "CosineAnnealingLR":
        # scheduler = LinearWarmupCosineAnnealingLR(optimizer, warmup_epochs=0, max_epochs=max_iters//batch_size, warmup_start_lr=0.00000, eta_min=eta_min)
        scheduler = eval(lr_type)(optimizer=optimizer, T_max=max_iters//batch_size, eta_min=eta_min)
    else:
        scheduler = "Fix"
...
def train(
    ...
    for iter_num in range(max_iters):
        ...
        if not is_accumulating:
            optimizer.step()
            optimizer.zero_grad()
            print("batch_loss:", losss/gradient_accumulation_iters)
            losss = 0
            step_count += 1
