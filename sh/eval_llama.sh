    ...
    do
        ...
        do
            ...
            if [[ $finetune == *lora* ]]; then
                ...
                cp checkpoints/meta-llama/$model/*.json \
                out/$model/$dataset/"$finetune"_"$optimizer"/$quantize/$ft_day &&
                cp checkpoints/meta-llama/$model/tokenizer.model \
                out/$model/$dataset/"$finetune"_"$optimizer"/$quantize/$ft_day
            fi
...
### 実行するとき
# CUDA_VISIBLE_DEVICES=0 nohup bash sh/eval_llama.sh 2>sh_logs/error_eval_llama.txt &