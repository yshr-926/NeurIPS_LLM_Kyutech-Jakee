"""Implementation derived from https://github.com/Muennighoff/FLAN"""
import json
import os
import sys
...
"""
python3 script/prepare_flan.py \
    --checkpoint_dir "checkpoints/EleutherAI/pythia-70m" \
    --destination_path data/pythia/pythia-70m/flan-test
"""
...
def prepare(
    ...
    data_repo_id: str = "Muennighoff/flan",
    ignore_index: int = -1,
    ignore_dataset: bool = True,
    access_token: Optional[str] = os.getenv("HF_TOKEN"),
    max_seq_length: Optional[int] = None,
) -> None:
    """Prepare the FLAN dataset for instruction tuning.
    ...
    train_data = format_dataset(dataset["train"], include_multiturn_conversations, ignore_dataset)
    ...
    # lima ではコメントアウトされていたが, flan では test データにも教師ラベルがあるからいけそう
    test_set = format_dataset(dataset["test"], include_multiturn_conversations, ignore_dataset)
...
def format_dataset(dataset_partition, include_multi_turn_conversations, ignore_dataset):
    formatted_ds = []
    ...
    for entry in dataset_partition:
        ...
        task = entry["task"]
        if ignore_dataset and task not in DATASET:
            continue
