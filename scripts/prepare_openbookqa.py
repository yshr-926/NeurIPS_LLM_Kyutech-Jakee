...
"""
python3 script/prepare_openbookqa.py \
    --checkpoint_dir "checkpoints/meta-llama/Llama-2-7b-hf" \
    --destination_path data/openbookqa-Llama-2-7b-hf \
    --max_seq_length 2048
"""

def prepare(
    destination_path: Path = Path("data/openbookqa"),
    test_split_fraction: float = 0.1,
    checkpoint_dir: Path = Path("checkpoints/stabilityai/stablelm-base-alpha-3b"),
    mask_inputs: bool = False,  # as in alpaca-lora
    seed: int = 42,
    include_multiturn_conversations: bool = False,
    data_repo_id: str = "openbookqa",
    ignore_index: int = -1,
    access_token: Optional[str] = os.getenv("HF_TOKEN"),
    max_seq_length: Optional[int] = None,
) -> None:
    """Prepare the OpenbookQAdataset for instruction tuning.
    ...
    train_set = format_dataset(dataset["train"], include_multiturn_conversations)
